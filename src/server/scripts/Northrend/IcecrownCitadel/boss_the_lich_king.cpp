...
enum Spells
{
    ...
    SPELL_TRIGGER_VILE_SPIRIT_HEROIC    = 73582,    /// @todo Cast every 3 seconds during Frostmourne phase, targets a Wicked Spirit and activates it
...
enum Events
{
	...
	EVENT_OUTRO_NO_REZ				= 38, // custom
    EVENT_OUTRO_TALK_1              = 39,
    EVENT_OUTRO_TALK_2              = 40,
    EVENT_OUTRO_EMOTE_TALK          = 41,
    EVENT_OUTRO_TALK_3              = 42,
    EVENT_OUTRO_MOVE_CENTER         = 43,
    EVENT_OUTRO_TALK_4              = 44,
    EVENT_OUTRO_RAISE_DEAD          = 45,
    EVENT_OUTRO_TALK_5              = 46,
    EVENT_OUTRO_BLESS               = 47,
    EVENT_OUTRO_REMOVE_ICE          = 48,
    EVENT_OUTRO_MOVE_1              = 49,
    EVENT_OUTRO_JUMP                = 50,
	EVENT_OUTRO_THROW_FROSTMOURNE	= 51, // custom
    EVENT_OUTRO_TALK_6              = 52,
    EVENT_OUTRO_KNOCK_BACK          = 53,
    EVENT_OUTRO_SOUL_BARRAGE        = 54,
    EVENT_OUTRO_SUMMON_TERENAS      = 55,
    EVENT_OUTRO_TERENAS_TALK_1      = 56,
    EVENT_OUTRO_TERENAS_TALK_2      = 57,
    EVENT_OUTRO_TALK_7              = 58,
    EVENT_OUTRO_TALK_8              = 59,
    ...
    // Raging Spirit
    EVENT_SOUL_SHRIEK               = 62,

    // Strangulate Vehicle (Harvest Soul)
    EVENT_TELEPORT                  = 63,
    EVENT_MOVE_TO_LICH_KING         = 64,
    EVENT_DESPAWN_SELF              = 65,
};
...
Position const Outro_Flying2        = {509.898682f, -2124.561035f, 850.356506f, 0.0f};
    ...
    public:
        ...
        struct boss_the_lich_kingAI : public BossAI
        {
            ...
            void JustDied(Unit* /*killer*/) OVERRIDE
            {
				...
				if (Creature* terenas = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_TERENAS_MENETHIL)))
				{
					terenas->DespawnOrUnsummon(8000);
				}
            ...
            void DoAction(int32 action) OVERRIDE
            {
                switch (action)
                {
                    ...
                    case ACTION_BREAK_FROSTMOURNE:
                        ...
                        events.ScheduleEvent(EVENT_OUTRO_TALK_6, 4000, 0, PHASE_OUTRO);
						events.ScheduleEvent(EVENT_OUTRO_THROW_FROSTMOURNE, 5000, 0, PHASE_OUTRO);
                        break;
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& /*damage*/) OVERRIDE
            {
                ...
                if (events.IsInPhase(PHASE_THREE) && !HealthAbovePct(10))
                {
					...
					events.ScheduleEvent(EVENT_OUTRO_NO_REZ, 2600, 0, PHASE_OUTRO);
                    events.ScheduleEvent(EVENT_OUTRO_TALK_1, 4000, 0, PHASE_OUTRO);
                    events.ScheduleEvent(EVENT_OUTRO_EMOTE_TALK, 8000, 0, PHASE_OUTRO);
                    events.ScheduleEvent(EVENT_OUTRO_EMOTE_TALK, 19000, 0, PHASE_OUTRO);
                    events.ScheduleEvent(EVENT_OUTRO_EMOTE_TALK, 29000, 0, PHASE_OUTRO);
                    events.ScheduleEvent(EVENT_OUTRO_TALK_2, 36000, 0, PHASE_OUTRO);
                    events.ScheduleEvent(EVENT_OUTRO_TALK_3, 45000, 0, PHASE_OUTRO);
                    events.ScheduleEvent(EVENT_EMOTE_CAST_SHOUT, 56000, 0, PHASE_OUTRO);
                    events.ScheduleEvent(EVENT_OUTRO_EMOTE_TALK, 60000, 0, PHASE_OUTRO);
                    events.ScheduleEvent(EVENT_OUTRO_MOVE_CENTER, 70000, 0, PHASE_OUTRO);
            ...
            void JustSummoned(Creature* summon) OVERRIDE
            {
                switch (summon->GetEntry())
                {
                    ...
                    case NPC_DRUDGE_GHOUL:
                        ...
                        summon->m_Events.AddEvent(new StartMovementEvent(me, summon), summon->m_Events.CalculateTime(4000));
                        break;
                    case NPC_ICE_SPHERE:
                    {
            ...
            void MovementInform(uint32 type, uint32 pointId) OVERRIDE
            {
                ...
                switch (pointId)
                {
                    case POINT_LK_INTRO_1:
                        ...
                        events.ScheduleEvent(EVENT_INTRO_MOVE_2, 0, 0, PHASE_INTRO);
                        break;
                    case POINT_LK_INTRO_2:
                        events.ScheduleEvent(EVENT_INTRO_MOVE_3, 0, 0, PHASE_INTRO);
                        break;
                    ...
                    case POINT_LK_OUTRO_1:
                        ...
                        events.ScheduleEvent(EVENT_OUTRO_TALK_5, 28999, 0, PHASE_OUTRO);
                        break;
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_INTRO_TALK_1:
                            ...
                            events.ScheduleEvent(EVENT_INTRO_CAST_FREEZE, 30400, 0, PHASE_INTRO);
                            break;
                        ...
                        case EVENT_SUMMON_DRUDGE_GHOUL:
                            ...
                            events.ScheduleEvent(EVENT_SUMMON_DRUDGE_GHOUL, 20000, 0, PHASE_ONE);
                            break;
                        case EVENT_INFEST:
                        ...
                        case EVENT_HARVEST_SOUL:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
							{
                                me->CastSpell(target, SPELL_HARVEST_SOUL, TRIGGERED_NONE);
							}
                        ...
                        case EVENT_HARVEST_SOULS:
                            ...
                            events.ScheduleEvent(EVENT_START_ATTACK, 50000);
                        ...
                        case EVENT_OUTRO_TALK_6:
                            ...
                            break;
						case EVENT_OUTRO_THROW_FROSTMOURNE:
							me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_ONESHOT_CUSTOM_SPELL_02);
							break;
                        case EVENT_OUTRO_SOUL_BARRAGE:
							me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_NONE);
							DoCast(me, SPELL_SOUL_BARRAGE);
                            ...
                            me->GetMotionMaster()->MovePoint(POINT_LK_OUTRO_2, Outro_Flying2);
                            break;
        ...
        struct npc_tirion_fordringAI : public ScriptedAI
        {
            ...
            void MovementInform(uint32 type, uint32 id) OVERRIDE
            {
                ...
                switch (id)
                {
                    ...
                    case POINT_TIRION_OUTRO_1:
                        _events.ScheduleEvent(EVENT_OUTRO_JUMP, 1000, 0, PHASE_OUTRO);
                        break;
                }
            }
            ...
            void SpellHit(Unit* /*caster*/, SpellInfo const* spell) OVERRIDE
            {
                if (spell->Id == SPELL_ICE_LOCK)
                    ...
                    SetEquipmentSlots(true);    // glow on ashbringer
            }
    ...
    public:
        ...
        struct npc_valkyr_shadowguardAI : public ScriptedAI
        {
            ...
            void Reset() OVERRIDE
            {
                ...
                me->SetSpeed(MOVE_FLIGHT, 0.50f, true);
            }
            ...
            void MovementInform(uint32 type, uint32 id) OVERRIDE
            {
                ...
                switch (id)
                {
                    ...
                    case POINT_CHARGE:
                        ...
                        {
							me->GetMotionMaster()->MovePoint(0, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), 0);
...
class spell_the_lich_king_jump_2 : public SpellScriptLoader
{
    public:
        spell_the_lich_king_jump_2() : SpellScriptLoader("spell_the_lich_king_jump_2") { }

        class spell_the_lich_king_jump_2_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_the_lich_king_jump_2_SpellScript);

            void HandleScript(SpellEffIndex effIndex)
            {
                PreventHitDefaultEffect(effIndex);
				GetCaster()->GetMotionMaster()->MoveJump(GetHitCreature()->GetPositionX()+5, GetHitCreature()->GetPositionY(), GetHitCreature()->GetPositionZ()+4, 20.0f, 20.0f);
            }

            void Register() OVERRIDE
            {
                OnEffectHitTarget += SpellEffectFn(spell_the_lich_king_jump_2_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };

        SpellScript* GetSpellScript() const OVERRIDE
        {
            return new spell_the_lich_king_jump_2_SpellScript();
        }
};
...
void AddSC_boss_the_lich_king()
{
	...
	new spell_the_lich_king_jump_2();
